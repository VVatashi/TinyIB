...
use Imageboard\Controllers\Admin\{
...
use Imageboard\Exceptions\{HttpException, NotFoundException};
...
class RoutingService implements RequestHandlerInterface
{
  ...
  function __construct(
    ...
    $this->dispatcher = \FastRoute\simpleDispatcher(function (RouteCollector $routes) {
      $routes->addGroup('/api', function (RouteCollector $routes) {
        ...
        $routes->addRoute('GET', '/status', [StatusApi::class, 'status']);
      });
   ...
   *
   * @throws \Imageboard\Exceptions\HttpException
   */
