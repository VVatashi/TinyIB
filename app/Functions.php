...
class Functions
{
   ...
   * @return string
   */
  static function plural(string $singular, int $count, string $plural = 's'): string
  {
    if ($plural === 's') {
   ...
   */
  static function manageCheckLogIn()
  {
    $loggedin = false;
    $isadmin = false;
   ...
   * @throws \Exception
   */
  static function validateFileUpload(array $file)
  {
    switch ($file['error']) {
      case UPLOAD_ERR_OK:
        break;
   ...
   *   Thumbnail size.
   */
  static function thumbnailDimensions(Post $post)
  {
    if ($post->isThread()) {
   ...
   * @return bool
   */
  static function createThumbnail($file_location, $thumb_location, $new_w, $new_h)
  {
   ...
   * @return bool
   */
  static function installedViaGit()
  {
    return is_dir('.git');
  }
